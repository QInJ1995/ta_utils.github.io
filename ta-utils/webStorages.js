...
class SealStorage {
    ...
    init(name = 'TA$webstorage', { isLocal = false, invalidTime = -1 }) {
        //
        if (isLocal) {
 ...
 */
export function createWebStorage(name = 'TA$webstorage', { isLocal = false, invalidTime = -1 } = {
    isLocal: false,
    invalidTime: -1,
}) {
...
export function init(...args) {
    console.error('[@yh/ta-utils]: WebStorages.init方法已经弃用,请使用WebStorages.createWebStorage代替');
...
export function getStorage(storageName, storageKey, isLocal = false) {
    ...
    if (webStorage === false) {
        console.error('[@yh/ta-utils]: 创建WebStorages失败!');
    }
