...
Schema.prototype = {
    ...
    asyncMap(series, options, function (data, doIt) {
      ...
      function cb () {
        ...
        if (!deep) {
          ...
          fieldsSchema = assign({}, fieldsSchema, data.rule.fields)
          for (let f in fieldsSchema) {
            if (fieldsSchema.hasOwnProperty(f)) {
              let fieldSchema = Array.isArray(fieldsSchema[f]) ? fieldsSchema[f] : [fieldsSchema[f]]
              fieldsSchema[f] = fieldSchema.map(addFullfield.bind(null, f))
            }
          }
          let schema = new Schema(fieldsSchema)
      ...
      let res = rule.validator(rule, data.value, cb, data.source, options)
      if (res && res.then) {
        res.then(function () {
          return cb()
        }, function (e) {
  ...
  getValidationMethod: function getValidationMethod (rule) {
    ...
    let messageIndex = keys.indexOf('message')
    if (messageIndex !== -1) {
